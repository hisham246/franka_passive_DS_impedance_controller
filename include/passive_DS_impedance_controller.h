...
#include <franka_passive_ds_impedance_controller/passive_ds_paramConfig.h>
...
class PassiveDSImpedanceController : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  // Dynamic reconfigure
  std::unique_ptr<dynamic_reconfigure::Server<franka_passive_ds_impedance_controller::passive_ds_paramConfig>>
      dynamic_server_passive_ds_param_;
  ros::NodeHandle dynamic_reconfigure_passive_ds_param_node_;
  void passiveDSParamCallback(franka_passive_ds_impedance_controller::passive_ds_paramConfig& config,
                               uint32_t level);
